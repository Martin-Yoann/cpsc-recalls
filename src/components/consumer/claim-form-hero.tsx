...
export function ClaimFormHero({ campaign }: Props) {
  ...
  const canAdvance = (): boolean => {
    ...
    if (step === 1) return form.lotCode.trim() !== "" && form.dateCode.trim() !== "";
      ...
      <div className="card-elevated p-6 sm:p-8">
        <div className="flex flex-col items-center text-center gap-4">
          ...
          <div className="w-full rounded-md bg-surface-dim p-4">
            ...
            <p className="mt-1 font-mono text-2xl font-bold text-foreground">{submit.claimNumber}</p>
          </div>
  ...
  return (
    <div className="card-elevated p-6 sm:p-8">
      ...
      <p className="label-eyebrow mb-4">Step {step + 1} of {STEPS.length} · {STEPS[step].label}</p>
      ...
      <div className="space-y-4 min-h-[260px]">
        {step === 0 && (
          <>
            ...
            <Field label="Phone Number" hint="Optional — for SMS status updates">
              <input
                type="tel"
                value={form.phone}
        ...
        {step === 1 && (
          <>
            <Field label="Lot Code" required hint="Printed near the package seal (e.g. ML-2406-A)">
              <input
                type="text"
                value={form.lotCode}
                onChange={(e) => update("lotCode", e.target.value.toUpperCase())}
        ...
        {step === 2 && (
          <div className="space-y-3">
            <p className="text-sm text-secondary">Select the remedy you would like to receive:</p>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2">
                    <p className="font-semibold text-foreground text-sm">{remedy.title}</p>
                  ...
                  <p className="mt-1 text-xs text-secondary leading-relaxed">{remedy.description}</p>
                </div>
              </label>
            ))}
          </div>
